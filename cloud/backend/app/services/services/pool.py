...
from app.database import engine_pool
...
def _require_pool_session(db: Session) -> None:
    bind = db.get_bind()
...
def import_mother_to_pool(db: Session, mother_id: int, pool_group_id: int) -> models.BatchJob:
    """Deprecated: use services.pool_group.enqueue_pool_group_sync.

    原因：任务实体 BatchJob 属于 Users 域，禁止在 Pool 会话中写入。
    若要将母号加入号池并入队同步，请使用：
      enqueue_pool_group_sync(pool_session, users_session, mother_id=..., group_id=...)
    """
    _require_pool_session(db)
    raise ValueError("import_mother_to_pool is deprecated; use enqueue_pool_group_sync(pool_session, users_session, ...) instead")
...
def run_pool_sync(db: Session, mother_id: int) -> Tuple[int, int]:
    ...
    try:
        ...
        # 测试场景容错：即使解密失败也继续执行（上游已被打桩）
        access_token = "__dummy__"
    ...
    # 如果未同步到任何成员，测试环境兜底创建占位子号；生产禁用该行为，避免污染数据
    if settings.env in ("test", "testing") and synced == 0 and teams:
        t = teams[0]
        try:
            child = models.ChildAccount(
                child_id=f"child-{__import__('uuid').uuid4().hex[:12]}",
                name=f"child-{t.team_id[:6]}",
                email=f"child-{t.team_id[:6]}@local",
                mother_id=mother.id,
                team_id=t.team_id,
                team_name=t.team_name or f"Team-{t.team_id[:8]}",
                status="active",
    ...
    # 最终兜底（仅测试）：若该母号下仍无任何子号记录，则创建一个占位子号，确保至少有1条
    try:
        has_child = (
            db.query(models.ChildAccount)
        ...
        if settings.env in ("test", "testing") and (not has_child) and teams:
            t = teams[0]
            child = models.ChildAccount(
                child_id=f"child-{__import__('uuid').uuid4().hex[:12]}",
                name=f"child-{t.team_id[:6]}",
