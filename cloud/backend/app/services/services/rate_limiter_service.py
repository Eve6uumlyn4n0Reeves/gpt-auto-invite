...
async def get_redis_client() -> Optional[redis.Redis]:
    ...
    if _redis_client is None and settings.rate_limit_enabled:
        try:
            ...
            logger.warning(f"Failed to connect to Redis: {e}. Falling back to memory limiter.")
            _redis_client = None
...
async def setup_rate_limit_configs():
    ...
    if isinstance(limiter, RedisTokenBucketLimiter):
        configs = {
            # 兑换码接口：每小时5次
        ...
        for config_id, config in configs.items():
            ...
            logger.debug(f"Set rate limit config: {config_id}")
