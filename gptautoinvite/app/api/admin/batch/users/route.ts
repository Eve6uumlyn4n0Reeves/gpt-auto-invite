...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call FastAPI backend for batch user operations
    const backendUrl = process.env.BACKEND_URL || "http://localhost:8000"
    const response = await fetch(`${backendUrl}/api/admin/batch/users`, {
      method: "POST",
      headers: {
    ...
    console.error("Batch users operation error:", error)
    return NextResponse.json(
      {
        success: false,
