...
def stats(request: Request, db: Session = Depends(get_db)):
    ...
    remaining_quota = max(0, capacity - active_codes)
    ...
    for mother in mothers:
        used = db.query(models.SeatAllocation).filter(
...
def dashboard_stats(request: Request, db: Session = Depends(get_db)):
    ...
    # 即将过期的兑换码
    expiring_soon = db.query(models.RedeemCode).filter(
        and_(
            models.RedeemCode.status != models.CodeStatus.used,
            models.RedeemCode.expires_at.isnot(None),
            models.RedeemCode.expires_at <= datetime.utcnow() + timedelta(days=7)
        )
    ).count()
