...
class BusinessMetrics:
    ...
    def _init_metric_definitions(self):
        ...
        self.metric_definitions = {
            ...
            "pool_capacity_total": MetricDefinition(
                name="pool_capacity_total",
                description="号池理论总席位",
                metric_type=MetricType.GAUGE,
            ),
            "pool_capacity_reserved": MetricDefinition(
                name="pool_capacity_reserved",
                description="已售兑换码数量",
                metric_type=MetricType.GAUGE,
            ),
            "pool_capacity_available": MetricDefinition(
                name="pool_capacity_available",
                description="剩余可售席位",
                metric_type=MetricType.GAUGE,
            ),
            "pool_capacity_warn": MetricDefinition(
                name="pool_capacity_warn",
                description="号池余量告警（0/1）",
                metric_type=MetricType.GAUGE,
            ),
        }
...
class DatabaseMetricsCollector:
    ...
    def collect_database_metrics(self, users_db: Session, pool_db: Session):
        ...
        try:
            ...
            # Capacity snapshot
            self._collect_capacity_metrics(users_db, pool_db)

        except Exception as e:
            import logging
            logging.getLogger(__name__).exception("收集数据库指标失败: %s", e)
    ...
    def _collect_capacity_metrics(self, users_db: Session, pool_db: Session):
        guard = CapacityGuard(users_db, pool_db)
        snapshot = guard.snapshot()
        business_metrics.set_gauge("pool_capacity_total", snapshot.total_slots)
        business_metrics.set_gauge("pool_capacity_reserved", snapshot.reserved_codes)
        business_metrics.set_gauge("pool_capacity_available", snapshot.available_slots)
        business_metrics.set_gauge("pool_capacity_warn", 1 if snapshot.warn else 0)
