...
def fetch_session_via_cookie(cookie: str) -> Tuple[str, Optional[datetime], Optional[str], Optional[str]]:
    ...
    Returns:
        tuple: (access_token, expires_at, email, team_id)

    Note:
        从session响应中提取:
        - access_token: 用于API调用的令牌
        - email: 用户邮箱
        - team_id: 账户ID (account.id)，即team_id
    ...
    team_id = account.get("id")  # account.id 就是 team_id
    ...
    if not team_id:
        raise ProviderError(500, "no_team_id", "No account/team id in session response")

    return token, token_expires_at, email, team_id
...
def list_teams(access_token: str, account_id: str, cursor: Optional[str] = None, limit: int = 50) -> dict:
    def _do():
        url = f"{BASE}/accounts/{account_id}/teams"
        params: dict[str, Any] = {"limit": limit}
        if cursor:
            params["cursor"] = cursor
        t0 = time.time()
        r = requests.get(url, headers=_headers(access_token, account_id), params=params, timeout=30, proxies=_proxies())
        provider_metrics.record("list_teams", account_id, r.status_code, (time.time() - t0) * 1000)
        if r.status_code != 200:
            raise ProviderError(r.status_code, "list_teams_failed", r.text[:1000])
        try:
            return r.json()
        except Exception as exc:
            raise ProviderError(r.status_code, "list_teams_invalid_json", str(exc)) from exc
    resp = _with_resilience(_do, 'list_teams', account_id)
    _record_success('list_teams', account_id)
    return resp
