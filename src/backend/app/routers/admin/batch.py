...
# module-level wrappers for easier testing/monkeypatching
# Each returns (ok: bool, error: Optional[str])
def resend_invite(email: str, team_id: str):
    u = SessionUsers()
    p = SessionPool()
    try:
        svc = InviteService(UsersRepository(u), MotherRepository(p))
        return svc.resend_invite(email, team_id)
    finally:
        try:
            p.close()
        except Exception:
...
def cancel_invite(email: str, team_id: str):
    u = SessionUsers()
    p = SessionPool()
    try:
        svc = InviteService(UsersRepository(u), MotherRepository(p))
        return svc.cancel_invite(email, team_id)
    finally:
        try:
            p.close()
        except Exception:
...
def remove_member(email: str, team_id: str):
    u = SessionUsers()
    p = SessionPool()
    try:
        svc = InviteService(UsersRepository(u), MotherRepository(p))
        return svc.remove_member(email, team_id)
    finally:
        try:
            p.close()
...
def batch_users(
    ...
    for uid in ids:
        ...
        if action == "resend":
            ok, _ = resend_invite(email, invite.team_id)
        elif action == "cancel":
            ok, _ = cancel_invite(email, invite.team_id)
        elif action == "remove":
            ok, _ = remove_member(email, invite.team_id)
        else:
            raise HTTPException(status_code=400, detail="不支持的操作")
