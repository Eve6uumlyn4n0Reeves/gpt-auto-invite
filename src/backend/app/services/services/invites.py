...
class InviteService:
    ...
    def _choose_target(
        self,
        email: str,
        exclude_mother_ids: Optional[set[int]] = None,
        ...
        *,
        ...
        order_column = (
            models.MotherAccount.created_at.desc()
            if prefer_recent
            else models.MotherAccount.created_at.asc()
        )
        mothers_query = (
        ...
        recent_cutoff = (
            datetime.utcnow() - timedelta(days=recent_window_days)
            if prefer_recent and recent_window_days
            else None
        )
        ...
        for mother in mothers_iter:
            ...
            if len(batch) >= batch_size:
                selection = self._choose_from_batch(
                    batch, email, prefer_recent=prefer_recent, recent_cutoff=recent_cutoff
                )
                if selection:
                    return selection
                batch.clear()
        ...
        if prefer_recent:
            return self._choose_target(
                email,
                exclude_mother_ids=exclude_mother_ids,
                group_id=group_id,
                prefer_recent=False,
            )
        return None
    ...
    def _choose_from_batch(
        self,
        mothers: Sequence[models.MotherAccount],
        email: str,
        *,
        prefer_recent: bool = False,
        ...
        for mother in mothers:
            ...
            if prefer_recent and recent_cutoff:
                filtered = [
                    t for t in available if t.created_at and t.created_at >= recent_cutoff
                ]
                if filtered:
                    available = filtered
            if not available:
    ...
    def invite_email(
        self,
        email: str,
        code_row: Optional[models.RedeemCode],
        *,
        ...
        if code_row and getattr(code_row, "mother_group_id", None):
            target_group_id = code_row.mother_group_id
        ...
        while True:
            choice = self._choose_target(
                email,
                exclude_mother_ids=tried_mothers,
                group_id=target_group_id,
                prefer_recent=prefer_recent_team,
    ...
    def resend_invite(self, email: str, team_id: str) -> tuple[bool, str]:
        ...
        mother = self.mother_repo.get(seat.mother_id)
...
def resend_invite(users_db: Session, pool_db: Session, email: str, team_id: str) -> tuple[bool, str]:
    service = InviteService(UsersRepository(users_db), MotherRepository(pool_db))
