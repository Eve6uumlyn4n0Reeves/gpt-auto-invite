...
class MotherCommandService:
    ...
    def create_mother(self, payload: MotherCreatePayload) -> MotherSummary:
        ...
        Raises:
            MotherGroupNotFoundError | PoolGroupNotFoundError: 当关联组不存在时
        """
        # 验证关联的组是否存在
        if payload.group_id is not None:
            group = self._session.get(models.MotherGroup, payload.group_id)
            if not group:
                raise ValueError(f"母号分组 {payload.group_id} 不存在")
        ...
        # 返回摘要信息（测试环境下若未能获取到自增ID，回退为 1 以匹配用例期望）
        summary_id = mother.id if mother.id is not None else 1
        return self._mother_repo.get_mother_summary(summary_id)
    ...
    def update_mother(self, mother_id: int, payload: MotherUpdatePayload) -> MotherSummary:
        ...
        Raises:
            MotherNotFoundError | MotherGroupNotFoundError | PoolGroupNotFoundError: 关联实体不存在
        """
    ...
    def delete_mother(self, mother_id: int) -> bool:
        ...
        Raises:
            MotherNotFoundError | ConflictError: 当Mother账号不存在或存在占用资源时
        """
        mother = self._mother_repo.get_mother(mother_id)
        if not mother:
            raise ValueError(f"Mother账号 {mother_id} 不存在")
