    ...
    async def extract_token_from_request(request: Request) -> Optional[str]:
        ...
        if request.method in ("POST", "PUT", "PATCH"):
            try:
                ...
                # 表单解析失败通常表示非表单请求，记录调试日志即可
                logger.debug("CSRF: failed to parse form for token extraction: %s", e)
        ...
        try:
            ...
            # JSON 解析失败也常见于非 JSON 请求体
            logger.debug("CSRF: failed to parse JSON for token extraction: %s", e)
