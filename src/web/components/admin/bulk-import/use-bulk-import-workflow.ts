...
import { validateMothersBulk, importMothersBulk } from '@/lib/api/mothers-bulk'
import {
  analyseDuplicates,
  createEntry,
  emptyTeam,
...
export const useBulkImportWorkflow = ({
  ...
  const validateEntries = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await validateMothersBulk(entries.map(serializeForApi))
      if (!('ok' in res) || !res.ok) {
        throw new Error(res.error || '校验失败，请稍后重试')
      }
      const data = (res.data || []) as Array<{ index: number; valid: boolean; warnings: string[]; teams: TeamFormInput[] }>
  ...
  const importEntries = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await importMothersBulk(entries.map(serializeForApi))
      if (!('ok' in res) || !res.ok) {
        throw new Error(res.error || '批量导入失败，请稍后重试')
      }
      const data = (res.data || []) as Array<{ index: number; success: boolean; error?: string }>
