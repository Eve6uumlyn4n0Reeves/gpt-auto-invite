...
import type { CodeStatus } from '@/shared/api-types'

const CODE_STATUS_TEXT: Record<CodeStatus, string> = {
  unused: '未使用',
  used: '已使用',
  expired: '已过期',
  blocked: '已封禁',
...
const CODE_STATUS_CLASS: Record<CodeStatus, string> = {
...
export const buildCodesTableColumns = ({
  ...
  },
  {
    ...
    render: (_: unknown, row: CodeData) => {
      ...
      return (
        <span className={`rounded-full border px-2 py-1 text-xs font-medium ${CODE_STATUS_CLASS[s]}`}>{
          CODE_STATUS_TEXT[s]
        }</span>
      )
    },
  ...
  },
  {
    ...
    render: (_: unknown, row: CodeData) => {
      const plan =
        row.lifecycle_plan === 'weekly'
          ? '7天'
          : row.lifecycle_plan === 'monthly'
            ? '30天'
            : '未设置'
      const expires = row.lifecycle_expires_at ? new Date(row.lifecycle_expires_at).toLocaleDateString() : '—'
      return (
  ...
  },
  {
    ...
    label: '切换次数',
    render: (_: unknown, row: CodeData) => {
      const count = row.switch_count ?? 0
      const limit = row.switch_limit ?? '∞'
      const activeLabel = row.active === false ? '已停用' : '可切换'
      return (
        <div className="text-xs leading-tight">
          <div>
            {count} / {limit}
          </div>
          <div className={row.active === false ? 'text-red-500' : 'text-green-600'}>{activeLabel}</div>
        </div>
      )
    },
  },
