...
const resolveBaseUrl = (envKey: string, fallback: string) => {
  if (isBrowser) {
    return ''
  }
  return process.env[envKey] || process.env.BACKEND_URL || fallback
}

export const getUsersApiBaseUrl = () => resolveBaseUrl('USERS_BACKEND_URL', 'http://localhost:8001')
export const getPoolApiBaseUrl = () => resolveBaseUrl('POOL_BACKEND_URL', 'http://localhost:8002')
export const getAdminApiBaseUrl = () => resolveBaseUrl('BACKEND_URL', 'http://localhost:8000')
...
export async function usersAdminRequest<T>(endpoint: string, options: AdminRequestOptions = {}) {
  ...
  return adminRequest<T>(endpoint, { ...options, headers }, getUsersApiBaseUrl())
}
...
export async function poolAdminRequest<T>(endpoint: string, options: AdminRequestOptions = {}) {
  ...
  return adminRequest<T>(endpoint, { ...options, headers }, getPoolApiBaseUrl())
}
